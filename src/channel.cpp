...
namespace rprotobuf {
	...
	SEXP invoke( SEXP method_xp, SEXP message_xp, SEXP channel_xp ){
		...
		MethodDescriptor* method = (MethodDescriptor*) EXTPTR_PTR( method_xp ) ;
		Message* message = (Message*) EXTPTR_PTR( message_xp ) ;
		Message* response = PROTOTYPE( method->output_type() )  ;
